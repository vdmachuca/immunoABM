function [L, TUcells, IMcells, TUprop, IMprop] = initializeSystem_2D(N,M,TUpmax)
    ...
    TUprop.isa = [true false false];
    TUprop.isb = [false true false];
    TUprop.isc = [false false true];

    
    %TUprop.isb = [true false];
    %TUprop.isa = [false true];
    
    TUprop.Pcap = [uint8(TUpmax) uint8(TUpmax) uint8(TUpmax)]; % set property of first cell: proliferation capacity
    TUprop.isStem = [true true true];        % set property of first cell: stemness
